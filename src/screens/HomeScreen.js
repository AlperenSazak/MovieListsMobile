...
export default function HomeScreen({ navigation }) {
    const { user, backdrops, currentBackdropIndex, fadeAnim } = useAuth();
    ...
    const loadPopularMovies = async (page = 1) => {
        try {
            ...
            setMyMovieIds(myIds);
            const filtered = (popular.results || []).filter(m => !myIds.includes(m.id));
    ...
    const renderMovie = ({ item }) => (
        ...
        >
            ...
            <View style={styles.movieInfo}>
                ...
                <Text style={styles.movieYear}>📅 {item.release_date?.split('-')[0] || 'N/A'}</Text>
                <Text style={styles.movieRating}>⭐ {item.vote_average?.toFixed(1) || 'N/A'}</Text>
                <Text style={styles.movieOverview} numberOfLines={3}>{item.overview || 'Açıklama bulunamadı.'}</Text>
                <View style={styles.buttonRow}>
                    <TouchableOpacity style={styles.addButton} onPress={() => handleAddMovie(item)}>
                        <Text style={styles.addButtonText}>➕ Kütüphaneye</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.watchLaterButton} onPress={() => handleWatchLater(item)}>
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.welcome}>Hoş geldin, {user?.username}! 👋</Text>
            ...
            <View style={styles.searchContainer}>
                ...
                >
                    <Text style={styles.genreBtnText}>{selectedGenre ? '✓' : '🎭'}</Text>
                </TouchableOpacity>
            </View>
            ...
            {loading ? (
                ...
                popularMovies.length > 0 ? (
                    <FlatList
                        ...
                        ListHeaderComponent={<Text style={styles.popularTitle}>🔥 Popüler Filmler</Text>}
                        ListFooterComponent={
                            <TouchableOpacity style={styles.loadMoreBtn} onPress={handleLoadMore} disabled={loadingMore}>
                                {loadingMore ? <ActivityIndicator color="#667eea" /> : <Text style={styles.loadMoreText}>Daha Fazla ↓</Text>}
                            </TouchableOpacity>
                        }
            ...
            <Modal visible={showGenreModal} transparent animationType="slide" onRequestClose={() => setShowGenreModal(false)}>
                <TouchableOpacity style={styles.modalOverlay} onPress={() => setShowGenreModal(false)}>
                    <View style={styles.modalContent}>
                        <Text style={styles.modalTitle}>🎭 Tür Seç</Text>
                        {selectedGenre && (
                            <TouchableOpacity style={styles.clearFilter} onPress={() => { setSelectedGenre(null); setSearched(false); setMovies([]); setShowGenreModal(false); }}>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            <View style={styles.genreGrid}>
                                {genres.map((genre) => (
                                    <TouchableOpacity
                                        key={genre.id}
                                        style={[styles.genreGridItem, selectedGenre?.id === genre.id && styles.genreGridItemActive]}
                                        onPress={() => { handleGenreSelect(genre); setShowGenreModal(false); }}
                                    >
                                        <Text style={[styles.genreGridText, selectedGenre?.id === genre.id && styles.genreGridTextActive]}>{genre.name}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </ScrollView>
...
const styles = StyleSheet.create({
    ...
    welcome: { color: '#fff', fontSize: 18, padding: 20, paddingBottom: 10 },
    toast: { padding: 15, borderRadius: 10, marginHorizontal: 20, marginBottom: 10 },
    ...
    searchContainer: { flexDirection: 'row', padding: 20, paddingTop: 10, gap: 10 },
    searchInput: { flex: 1, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10, padding: 12, color: '#fff', fontSize: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)', outlineStyle: 'none' },
    searchBtn: { backgroundColor: '#667eea', borderRadius: 10, padding: 12, justifyContent: 'center', alignItems: 'center', width: 50 },
    searchBtnText: { fontSize: 20 },
    loader: { marginTop: 50 },
    emptyText: { color: '#888', textAlign: 'center', marginTop: 50, fontSize: 18 },
    ...
    movieRating: { color: '#f093fb', fontSize: 13, marginBottom: 5 },
    movieOverview: { color: '#aaa', fontSize: 12, lineHeight: 18, marginBottom: 10 },
    addButton: { flex: 1, backgroundColor: 'rgba(102, 126, 234, 0.3)', borderWidth: 1, borderColor: 'rgba(102, 126, 234, 0.5)', borderRadius: 8, padding: 8, alignItems: 'center' },
    addButtonText: { color: '#667eea', fontSize: 11, fontWeight: '600' },
    buttonRow: { flexDirection: 'row', gap: 8, marginTop: 8 },
    watchLaterButton: { flex: 1, backgroundColor: 'rgba(240, 147, 251, 0.2)', borderWidth: 1, borderColor: 'rgba(240, 147, 251, 0.5)', borderRadius: 8, padding: 8, alignItems: 'center' },
    watchLaterButtonText: { color: '#f093fb', fontSize: 11, fontWeight: '600' },
    popularTitle: { color: '#fff', fontSize: 20, fontWeight: 'bold', marginBottom: 15 },
    loadMoreBtn: { backgroundColor: 'rgba(102, 126, 234, 0.2)', borderWidth: 1, borderColor: 'rgba(102, 126, 234, 0.4)', borderRadius: 12, padding: 15, alignItems: 'center', marginTop: 10, marginBottom: 20 },
    loadMoreText: { color: '#667eea', fontSize: 16, fontWeight: '600' },
    genreBtn: { backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10, padding: 12, justifyContent: 'center', alignItems: 'center', width: 50, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
    genreBtnActive: { backgroundColor: 'rgba(102, 126, 234, 0.4)', borderColor: '#667eea' },
    genreBtnText: { fontSize: 20 },
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'flex-end' },
    modalContent: { backgroundColor: '#1a1a1a', borderTopLeftRadius: 25, borderTopRightRadius: 25, padding: 25, maxHeight: '70%' },
    modalTitle: { color: '#fff', fontSize: 20, fontWeight: 'bold', marginBottom: 15, textAlign: 'center' },
    clearFilter: { backgroundColor: 'rgba(220, 53, 69, 0.2)', borderWidth: 1, borderColor: 'rgba(220, 53, 69, 0.4)', borderRadius: 10, padding: 10, alignItems: 'center', marginBottom: 15 },
    clearFilterText: { color: '#dc3545', fontWeight: '600' },
    genreGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
    genreGridItem: { backgroundColor: 'rgba(255,255,255,0.1)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)', borderRadius: 20, paddingHorizontal: 15, paddingVertical: 10 },
    genreGridItemActive: { backgroundColor: 'rgba(102, 126, 234, 0.4)', borderColor: '#667eea' },
    genreGridText: { color: '#888', fontSize: 14, fontWeight: '600' },
    genreGridTextActive: { color: '#667eea' },
});